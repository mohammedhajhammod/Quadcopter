...
float pid_i_gain_roll = 0.0005;              //Gain setting for the roll I-controller //0.04
float pid_d_gain_roll = 8.0;              //Gain setting for the roll D-controller //18.0 
...
float pid_i_gain_yaw = 0.0004;               //Gain setting for the pitch I-controller. //0.02
...
void setup(){
 ...
 //Use the led on the Arduino for startup indication.
  ...
  for (cal_int = 0; cal_int < 2500 ; cal_int ++){                           //Take 2000 readings for calibration.
  ...
  gyro_roll_cal /= 2500;                                                 //Divide the roll total by 2000.
  gyro_pitch_cal /= 2500;                                                 //Divide the pitch total by 2000.
  gyro_yaw_cal /= 2500;                                                 //Divide the yaw total by 2000.
  ...
  //11.65V equals ~5V @ Analog 0.
  //11.65V equals 1023 analogRead(0).
  //1165 / 1023 = 1.1388.
  //The variable battery_voltage holds 1050 if the battery voltage is 10.5V.
  battery_voltage = analogRead(0) * 1.1388;
...
void loop(){
  ...
  angle_pitch_acc -= 1.32;                                                   //Accelerometer calibration value for pitch.
  ...
  //0.0911 = 0.08 * 1.1388.
  battery_voltage = battery_voltage * 0.92 + analogRead(0) * 0.0911;
